...
public class ChatHandler extends SimpleChannelInboundHandler<TextWebSocketFrame> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, TextWebSocketFrame msg) throws Exception {
        ...
        System.out.println("客户端发送来的消息: " + text);

        //写法1
//        for (Channel channel : clients) {
//            channel.writeAndFlush(new TextWebSocketFrame("[服务器在: ]" + LocalDateTime.now() + ",接收到的消息消息为: " + text));
//        }

        //写法2
        clients.writeAndFlush(new TextWebSocketFrame("[服务器在: ]" + LocalDateTime.now() + ",接收到的消息消息为: " + text));
